...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter{
	...
	private final String ROLES_QUERY="select u.username, r.role from user u inner join user_role ur on (u.id=ur.user_id) "+
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		.authoritiesByUsernameQuery(ROLES_QUERY).dataSource(dataSource)
    ...
    protected void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers("/signup").permitAll().antMatchers("/prediction/**").hasAuthority("USER")
		.anyRequest().authenticated().and().csrf().disable().formLogin()
		.loginPage("/login").failureUrl("/login?error=true").defaultSuccessUrl("/prediction/home")
